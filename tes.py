...
def process_symbol(symbol, state):
    ...
    per_5m_open = trend_5m["HA_open"].iloc[-3]
    per_5m_close = trend_5m["HA_close"].iloc[-3]
    ...
    if pos is None and state["last_candle"] != candle_time:
        ...
        if last_5m_close > one_hour_trendline and last_5m_close > per_5m_open and last_5m_close > per_5m_close:
            state["position"] = {
                "side": "long",
                "entry": price,
