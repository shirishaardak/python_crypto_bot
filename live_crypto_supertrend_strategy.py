        ...
        if now.second == 10 and now.minute % 15 == 0:
            ...
            for symbol, product_id in symbols_map.items():
                ...
                # --- BUY MANAGEMENT ---
                        ...
                        if stop_id:
                            ...
                            if stop_still_pending:
                                ...
                                if cancel_result:
                                    ...
                                    log(f"⚠️ Failed to cancel stop order {stop_id} for {symbol}", alert=True)
                            else:
                                log(f"ℹ️ Stop order {stop_id} for {symbol} already executed or not found", alert=False)
                    ...
                    # Check if stop loss was already triggered (stop order executed)
                    elif stop_id:
                        live_orders = get_live_orders_with_error_handling()
                        stop_still_active = any(o['id'] == stop_id for o in live_orders)
                        
                        if not stop_still_active:
                            # Stop loss was triggered - position already closed
                            exit_price = renko_param[symbol]['EMA_21_DN']  # Approximate exit price
                            entry_price = renko_param[symbol]['entry_price']
                            pnl = (exit_price - entry_price) * QTY[symbol]
                ...
                # --- SELL MANAGEMENT ---
                    ...
                    # Check if stop loss was already triggered (stop order executed)
                        ...
                        if not stop_still_active:
                            # Stop loss was triggered - position already closed
                            exit_price = renko_param[symbol]['EMA_21_UP']  # Approximate exit price
                            entry_price = renko_param[symbol]['entry_price']
                            pnl = (entry_price - exit_price) * QTY[symbol]
