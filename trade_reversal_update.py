...
def process_price_trend(symbol, price, positions, last_base_price,
    ...
    if pos is not None:
        ...
        # If trendline made new swing → update trailing stop
        if raw_trendline == df["HA_high"].iloc[-2]:
            new_stop = df["HA_low"].iloc[-2]
            if new_stop > pos["stop"]:
                pos["stop"] = new_stop
                log(f"{symbol} | LONG STOP UPDATE | Stop:{pos['stop']}")
           

        # Always enforce directional movement (stop cannot go down)
        pos["stop"] = max(pos["stop"], pos["trendline"])
    ...
    if pos is not None:
        ...
        if raw_trendline == df["HA_low"].iloc[-2]:
            ...
            if new_stop < pos["stop"]:
                pos["stop"] = new_stop
                log(f"{symbol} | SHORT STOP UPDATE | Stop:{pos['stop']}")
            

        # Stop cannot move up for a short position
        pos["stop"] = min(pos["stop"], pos["trendline"])
